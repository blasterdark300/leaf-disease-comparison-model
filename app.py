...
if menu == "Deteksi Penyakit":
    ...
    if image:
        ...
        if st.button("Analisis 3 Model Sekaligus"):
            ...
            for i, (key, path) in enumerate(model_dict.items()):
                with cols[i]:
                    st.info(f"Model: {key}")
                    ...
                    # Fitur Validasi
                    valid = st.radio(f"Validasi {key.split('/')[0]}", ["Belum", "Benar", "Salah"], key=f"val_{key}")
                    
                    if st.button(f"Simpan Hasil {key.split('/')[0]}", key=f"btn_{key}"):
                        st.session_state.history.append({"Model": key, "Hasil": label, "Validasi": valid})
...
# --- HALAMAN LAINNYA ---
    ...
    if 'history' in st.session_state and st.session_state.history:
        ...
        st.download_button("📥 Unduh CSV Validasi", csv, "hasil_validasi.csv", "text/csv")
    else:
        st.write("Belum ada data yang divalidasi.")
else:
